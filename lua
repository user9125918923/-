...
local SERVICE_NAME = "EnhancedMovementService"
local REMOTE_NAMES = {
    "RunStateToggled",
    "DoubleJumped",
    "RanToTarget",
...
local function hardReplaceEnhancedMovement()
    local existing = Remotes:FindFirstChild(SERVICE_NAME)
    if existing then
        pcall(function() existing:Destroy() end)
    end

    local dummy = Instance.new("Folder")
    dummy.Name = SERVICE_NAME
    dummy.Parent = Remotes
...
hardReplaceEnhancedMovement()

Remotes.ChildAdded:Connect(function(child)
    if child.Name == SERVICE_NAME then
        task.wait()
        hardReplaceEnhancedMovement()
    end
end)
